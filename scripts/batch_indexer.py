...
def get_shell_history():
    history = []
    # ZSH History (often has timestamps : 1234567890:0;cmd)
            ...
            with open(zsh_path, "r", errors="ignore") as f:
                for line in f:
                    if "; " in line:
                        ...
                        # Handle multiline zsh history or other formats
                        parts = line.split(";", 1)
                        if len(parts) > 1:
...
def run_full_index():
    ...
    console.print("[dim]Scanning historical intent vectors and shell signatures...[/]\n")

    # 1. Gather candidates
    history = get_shell_history()
    
    # 2. Filter already existing (check memory.db content)
    ...
    if not new_items:
        console.print("[green]Neural parity achieved. No new historical intent detected.[/]")
        return

    console.print(f"[bold green]Detected {len(new_items)} new historical intent units.[/]")
    
    # Limit to reasonable batches if huge
    limit = 1000 # Let's not do 6000 at once to avoid API timeouts
    if len(new_items) > limit:
        console.print(f"[yellow]Performance Cap:[/] Truncating to the latest {limit} units for this session.")
        ...
        for i in range(0, len(new_items), batch_size):
            ...
            for text, emb in zip(batch, embeddings):
                ...
                conn.execute(
                    ...
                    ("SHELL_HISTORY", text, "Shell History Import", 3, emb_blob)
                )
            ...
            # Sync to vector_registry.json in batch
            memory.add_to_registry_batch(batch_data_json)
            
            progress.update(task, advance=len(batch))

    console.print(f"\n[bold green]Success:[/ ] Ingested {len(new_items)} shell commands into 768D semantic space.")
    console.print("[dim]Run again to ingest the next batch of historical signatures.[/dim]\n")
